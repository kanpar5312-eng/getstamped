...
<body style="margin:0;padding:0;background:#f5efe4;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Inter,sans-serif;color:#14211c;">
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0">
    <tr><td align="center" style="padding:40px 16px;">
      <table role="presentation" width="100%" style="max-width:560px;background:#faf6ed;border:1px solid #ede4d2;border-radius:16px;padding:32px;text-align:left;">
        <tr><td>
          <div style="font-size:11px;letter-spacing:0.18em;text-transform:uppercase;color:#6a766f;font-weight:500;">
            GetStamped
          </div>
          ${inner}
          <hr style="border:none;border-top:1px solid #ede4d2;margin:32px 0 16px 0;" />
          <p style="font-size:12px;color:#6a766f;margin:0;">
            GetStamped · F-1 visa preparation, end to end.<br />
            <a href="https://getstamped.app" style="color:#1c86b8;text-decoration:none;">getstamped.app</a>
          </p>
        </td></tr>
      </table>
    </td></tr>
...
GetStamped`;
  const html = htmlShell(`
    <h1 style="font-size:24px;margin:8px 0 0 0;font-weight:500;color:#14211c;">
      You're #${position} on the waitlist
    </h1>
    <p style="font-size:15px;line-height:1.6;color:#2a3a33;margin-top:16px;">${priceLine}</p>
    <p style="font-size:15px;line-height:1.6;color:#2a3a33;">We launch in about 6 weeks. You'll get access first.</p>
    <p style="font-size:15px;line-height:1.6;color:#2a3a33;">Reply to this email with any questions. I read every one.</p>
    <p style="font-size:15px;line-height:1.6;color:#2a3a33;margin-top:24px;">— ${founderName}</p>
  `);
...
GetStamped`;
  const html = htmlShell(`
    ...
    <p style="font-size:15px;line-height:1.6;color:#2a3a33;margin-top:16px;">Your account is live. Three things to do first:</p>
    <ol style="font-size:15px;line-height:1.7;color:#2a3a33;padding-left:20px;">
      <li>Complete onboarding (country + intake + program) — 90 seconds.</li>
      <li>Open your <a href="https://getstamped.app/dashboard/timeline" style="color:#143a2f;font-weight:500;">timeline</a>. All 47 F-1 steps, ordered.</li>
      <li>Mark Step 1 complete when you've gathered your offer + I-20 ID.</li>
    </ol>
    <p style="font-size:15px;line-height:1.6;color:#2a3a33;">If anything looks broken or confusing, reply to this email. I read every one.</p>
    <p style="font-size:15px;line-height:1.6;color:#2a3a33;margin-top:24px;">— ${FOUNDER}</p>
  `);
  return { subject, text, html };
}
...
— ${FOUNDER}`;
  const html = htmlShell(`
    <h1 style="font-size:22px;margin:8px 0 0 0;font-weight:500;color:#14211c;">
      Step ${opts.stepNumber} is waiting
    </h1>
    <p style="font-size:15px;line-height:1.6;color:#2a3a33;margin-top:16px;">
    ...
    <p style="margin-top:24px;">
      <a href="https://getstamped.app/dashboard/timeline/${opts.stepNumber}"
         style="display:inline-block;background:#143a2f;color:#faf6ed;padding:10px 20px;border-radius:8px;text-decoration:none;font-size:14px;font-weight:500;">
        Resume Step ${opts.stepNumber}
      </a>
    </p>
    <p style="font-size:12px;color:#6a766f;margin-top:24px;">
      You can mute reminders in Settings → Notifications.
    </p>
  `);
  return { subject, text, html };
...
— ${FOUNDER}`;
  const html = htmlShell(`
    ...
    <div style="margin-top:20px;border:1px solid #ede4d2;border-radius:12px;padding:20px;background:#ffffff;">
      <p style="margin:0;font-size:13px;color:#6a766f;text-transform:uppercase;letter-spacing:0.14em;font-weight:500;">Completed</p>
      <p style="margin:6px 0 0 0;font-size:32px;color:#143a2f;font-weight:500;">${opts.stepsCompletedThisWeek}<span style="font-size:14px;color:#6a766f;"> steps</span></p>
    </div>
    <ul style="font-size:15px;line-height:1.7;color:#2a3a33;padding-left:20px;margin-top:16px;">
...
— GetStamped`;
  const html = htmlShell(`
    ...
    <p style="font-size:15px;line-height:1.6;color:#2a3a33;">
    ...
    <p style="font-size:12px;color:#6a766f;margin-top:16px;word-break:break-all;">${opts.shareUrl}</p>
  `);
  return { subject, text, html };
}
...
<html><body style="margin:0;padding:0;background:#f5efe4;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Inter,sans-serif;color:#14211c;">
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0">
    <tr><td align="center" style="padding:32px 16px;">
      <table role="presentation" width="100%" style="max-width:560px;background:#faf6ed;border:1px solid #ede4d2;border-radius:16px;padding:24px;text-align:left;">
        <tr><td>
          <div style="font-size:11px;letter-spacing:0.18em;text-transform:uppercase;color:#6a766f;font-weight:500;">
            GetStamped · Contact form
          </div>
          <h1 style="font-size:20px;margin:8px 0 0 0;font-weight:500;color:#14211c;">
            ${opts.category.toUpperCase()} · ${opts.name}
          </h1>
          <table cellpadding="6" cellspacing="0" border="0" style="margin-top:16px;font-size:13px;color:#2a3a33;">
            <tr><td style="color:#6a766f;">Email</td><td><a href="mailto:${opts.email}" style="color:#1c86b8;text-decoration:none;">${opts.email}</a></td></tr>
            <tr><td style="color:#6a766f;">Category</td><td>${opts.category}</td></tr>
          </table>
          <hr style="border:none;border-top:1px solid #ede4d2;margin:16px 0;" />
          <p style="font-size:14px;line-height:1.6;color:#14211c;white-space:pre-wrap;margin:0;">${opts.message.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;")}</p>
        </td></tr>
      </table>
    </td></tr>
  </table>
...
<html><body style="margin:0;padding:0;background:#f5efe4;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Inter,sans-serif;color:#14211c;">
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0">
    <tr><td align="center" style="padding:40px 16px;">
      <table role="presentation" width="100%" style="max-width:560px;background:#faf6ed;border:1px solid #ede4d2;border-radius:16px;padding:32px;text-align:left;">
        <tr><td>
          <div style="font-size:11px;letter-spacing:0.18em;text-transform:uppercase;color:#6a766f;font-weight:500;">GetStamped</div>
          <h1 style="font-size:24px;margin:8px 0 0 0;font-weight:500;color:#14211c;">Got your message, ${opts.name}.</h1>
          <p style="font-size:15px;line-height:1.6;color:#2a3a33;margin-top:16px;">Your <strong>${opts.category}</strong> request reached me. I'll reply <strong>${window}</strong>.</p>
          <p style="font-size:15px;line-height:1.6;color:#2a3a33;">If it's urgent (visa interview in the next 48 hours, document issue, payment problem), reply to this email with <strong>URGENT</strong> in the subject and I'll bump it to the front.</p>
          <p style="font-size:15px;line-height:1.6;color:#2a3a33;margin-top:24px;">— Parneet</p>
          <hr style="border:none;border-top:1px solid #ede4d2;margin:24px 0 12px 0;" />
          <p style="font-size:11px;color:#6a766f;margin:0;">This is an automated acknowledgement — a real reply follows.</p>
        </td></tr>
      </table>
    </td></tr>
  </table>
