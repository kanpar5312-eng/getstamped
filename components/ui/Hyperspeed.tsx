...
const Hyperspeed = ({ effectOptions = DEFAULT_EFFECT_OPTIONS }: HyperspeedProps) => {
  ...
  useEffect(() => {
    ...
    class App {
      ...
      renderer: any; composer: any; camera: any; scene: any;
      fogUniforms: any; clock: any; assets: any; disposed: boolean;
      ...
      tick() {
        ...
        if (this.hasValidSize && !this.isPaused) {
          ...
          // keep clock from accumulating delta while paused
          this.clock.getDelta();
        }
    ...
    // Pause when off-screen or tab hidden — keeps the Mac cool.
    const io = new IntersectionObserver(
      (entries) => {
        for (const e of entries) {
          myApp.isPaused = !e.isIntersecting;
        }
      },
      { threshold: 0 }
    );
    ...
    const onVis = () => {
      if (document.hidden) myApp.isPaused = true;
      else {
        // resume only if currently on-screen
        const rect = container.getBoundingClientRect();
        const onScreen = rect.bottom > 0 && rect.top < window.innerHeight;
        myApp.isPaused = !onScreen;
      }
    };
