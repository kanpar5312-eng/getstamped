...
export default function FloatingLines({
  ...
  // Merge gradient props into a single array. linesGradient takes priority;
  // otherwise we fall back to gradientStart/Mid/End if provided.
  ...
  useEffect(() => {
    ...
    // Cap DPR — Mac safety; high-DPR retina can otherwise melt the GPU on
    // a full-bleed hero.
    ...
    if (resolvedGradient.length > 0) {
      ...
      stops.forEach((hex: string, i: number) => {
        ...
        (uniforms.lineGradient.value[i] as Vector3).set(color.x, color.y, color.z);
      });
    }
    ...
    // Mac safety: pause when the hero scrolls off-screen + when the tab
    // is hidden. Both common scenarios for users keeping the site open.
    const io = new IntersectionObserver(
      (entries) => {
        for (const e of entries) isPaused = !e.isIntersecting;
      },
      { threshold: 0 },
    );
    ...
    const renderLoop = () => {
      ...
      if (!isPaused) {
        ...
        // Keep the clock from accumulating delta while paused.
        clock.getDelta();
      }
